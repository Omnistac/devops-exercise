...
import { healthLoggingMiddleware, registerHealthChecks } from "./lib";
...
describe("Health Module", () => {
	describe("healthLoggingMiddleware", () => {
		...
		it("should log health check requests when completed", async () => {
			...
			assert.ok(
				loggedMessages[0].includes("/readiness"),
				"Expected log to include path",
			);
		});
		...
		it("should not log non-health requests", async () => {
			...
			assert.equal(
				loggedMessages.length,
				0,
				"Expected no messages to be logged",
			);
		});
	});
});
