...
describe("Trading Service - Performance Test Suite", () => {
	...
	before(async () => {
		...
		console.log(
			`Preparing test data with ${Object.keys(TEST_STOCKS).length} stocks...`,
		);

		// Write test stock data to file
		fs.writeFileSync(
			TEST_STOCKS_JSON_PATH,
			JSON.stringify(TEST_STOCKS),
			"utf-8",
		);
		...
		app.post("/swap", (req, res) => {
			...
			try {
				...
				if (stock.owned !== fromUserId) {
					return res
						.status(400)
						.json({ error: "Stock is not owned by the specified user" });
				}
		...
		app.post("/bulk-swap", (req, res) => {
			try {
				...
				if (!Array.isArray(operations) || operations.length === 0) {
					return res
						.status(400)
						.json({ error: "Missing or invalid operations array" });
				}
		...
		app.get("/portfolio/:userId", (req, res) => {
			...
			const userStocks = Object.values(stocks).filter(
				(stock: any) => stock.owned === userId,
			);

			// Calculate portfolio value
			const totalValue = userStocks.reduce(
				(sum: number, stock: any) => sum + stock.price,
				0,
			);
		...
		app.get("/sector-stats", (req, res) => {
			...
			Object.keys(sectorStats).forEach((sector) => {
				sectorStats[sector].avgPrice =
					sectorStats[sector].totalValue / sectorStats[sector].count;
			});
	...
	describe("Stock Swapping Operations", () => {
		it("should successfully swap stock ownership", async () => {
			testStats.testsRun++;
			const response = await request(app).post("/swap").send({
				stockId: "AAPL",
				fromUserId: "user1",
				toUserId: "user2",
			});
			...
			const updatedStocks = JSON.parse(
				fs.readFileSync(TEST_STOCKS_JSON_PATH, "utf-8"),
			);
	...
	describe("Bulk Operations", () => {
		...
		it("should handle large bulk operations (50 operations)", async () => {
			...
			const successes = response.body.results.filter(
				(r: any) => r.success,
			).length;
			const failures = response.body.results.filter(
				(r: any) => !r.success,
			).length;
			...
			const validOpsCreated = operations.filter(
				(op) => stockOwnership[op.stockId],
			).length;
			assert.ok(
				successes >= validOpsCreated * 0.9,
				`Expected at least 90% of valid operations (${validOpsCreated}) to succeed, but only got ${successes}`,
			);
		...
		it("should return correct portfolio for a user", async () => {
			...
			for (const stockId of stocksToAssign) {
				...
				if (currentOwner !== "user1") {
					await request(app).post("/swap").send({
						stockId,
						fromUserId: currentOwner,
						toUserId: "user1",
					});
			...
			for (const stockId of stocksToAssign) {
				const stockInPortfolio = response.body.stocks.find(
					(s: any) => s.id === stockId,
				);
				assert.ok(
					stockInPortfolio,
					`Stock ${stockId} should be in user1's portfolio`,
				);
			}
		});
	...
	describe("Sector Analysis", () => {
		it("should return correct sector statistics", async () => {
			...
			assert.ok(
				...
				"Expected Technology sector to exist",
			);
	...
	describe("Complex Scenarios", () => {
		it("should handle a trading simulation between multiple users", async () => {
			...
			if (!user1HasStock) {
				...
				const swapResponse = await request(app).post("/swap").send({
					stockId,
					fromUserId: currentOwner,
					toUserId: "user1",
				});
